class FileInput < SimpleForm::Inputs::Base
  def input
    ...
    button = template.content_tag(:div, class: 'input-append') do
      ...
      template.content_tag(:a, "Browse", class: 'btn btn-info', onclick: "$('input[id=#{idf}]').click();")
    end

    script = template.content_tag(:script, type: 'text/javascript') do
      "$(function(){ $('input[id=#{idf}]').change(function() { s = $(this).val(); $('#pbox_#{idf}').val(s.slice(s.lastIndexOf('\\\\\\\\')+1)); }); });".html_safe
    end
